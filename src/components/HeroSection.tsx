...
const HeroSection = () => {
  ...
  return (
    <header className="w-full p-6 md:p-8 pl-8 md:pl-10">
      ...
      >
        <span
          className={`block w-6 h-[1.5px] bg-foreground transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-[3.75px]" : ""}`}
        />
        <span
          className={`block w-6 h-[1.5px] bg-foreground transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-[3.75px]" : ""}`}
        />
      ...
      >
        {navItems.map((item) => (
          <button
            ...
            className="text-foreground hover:underline underline-offset-8 decoration-2 decoration-[hsl(210,100%,56%)] transition-all"
          >
            {item.label}
          </button>
        ))}
